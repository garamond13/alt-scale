...
vec4 hook()
{
	...
	for (float i = 1.0; i <= R; ++i) {
		weight = get_weight(i);
		csum += (PASS5_tex(PASS5_pos + vec2(0.0, -i) * PASS5_pt).x + PASS5_tex(PASS5_pos + vec2(0.0, i) * PASS5_pt).x) * weight;
		wsum += 2.0 * weight;
	}
	return vec4(csum / wsum, 0.0, 0.0, 0.0);
}
...
{
	float weight;
	float csum = PASS6_tex(PASS6_pos).x;
	float wsum = 1.0;
	for (float i = 1.0; i <= R; ++i) {
		weight = get_weight(i);
		csum += (PASS6_tex(PASS6_pos + vec2(-i, 0.0) * PASS6_pt).x + PASS6_tex(PASS6_pos + vec2(i, 0.0) * PASS6_pt).x) * weight;
		wsum += 2.0 * weight;
	}
	float original = PASS5_tex(PASS5_pos).x;
