...
//
#define K LINEAR //kernel filter, see "KERNEL FILTERS LIST"
...
vec4 hook() {
    ...
    for (float i = 1.0 - r; i <= r; ++i) {
        ...
        csum += textureLod(HOOKED_raw, base + HOOKED_pt * vec2(0.0, i), 0.0) * HOOKED_mul * weight;
        wsum += weight;
    }
    return csum / wsum;
}
...
vec4 hook() {
    ...
    for (float i = 1.0 - r; i <= r; ++i) {
        weight = get_weight(abs((i - fcoord) / scale));
        csum += textureLod(PASS0_raw, base + PASS0_pt * vec2(i, 0.0), 0.0) * PASS0_mul * weight;
        wsum += weight;
    }
    return csum / wsum;
}
...
vec4 hook() {
    ...
    vec4 original = textureLod(PASS1_raw, PASS1_pos, 0.0) * PASS1_mul;
