...
vec4 hook() {
    float weight;
    vec4 csum = textureLod(HOOKED_raw, HOOKED_pos, 0.0) * HOOKED_mul;
    float wsum = 1.0;
    for(float i = 1.0; i <= R; ++i) {
        weight = get_weight(i);
        csum += (textureLod(HOOKED_raw, HOOKED_pos + HOOKED_pt * vec2(0.0, -i), 0.0) + textureLod(HOOKED_raw, HOOKED_pos + HOOKED_pt * vec2(0.0, i), 0.0)) * HOOKED_mul * weight;
        wsum += 2.0 * weight;
    }
    return csum / wsum;
}
...
vec4 hook() {
    ...
    for(float i = 1.0; i <= R; ++i) {
        weight = get_weight(i);
        csum += (textureLod(PASS0_raw, PASS0_pos + PASS0_pt * vec2(-i, 0.0), 0.0) + textureLod(PASS0_raw, PASS0_pos + PASS0_pt * vec2(i, 0.0), 0.0)) * PASS0_mul * weight;
        wsum += 2.0 * weight;
    }
    return csum / wsum;
}
...
vec4 hook() {
    ...
    return original + (original - csum / wsum) * A;
}
