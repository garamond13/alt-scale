...
vec4 hook() {
    float f = fract(HOOKED_pos.y * input_size.y - 0.5);
    vec2 base = HOOKED_pos - f * HOOKED_pt * vec2(0.0, 1.0);
    vec4 color;
    float weight;
    vec4 csum = vec4(0.0);
    float wsum = 0.0;
...
vec4 hook() {
    float f = fract(PASS1_pos.x * input_size.x - 0.5);
    vec2 base = PASS1_pos - f * PASS1_pt * vec2(1.0, 0.0);
    vec4 color;
    float weight;
    vec4 csum = vec4(0.0);
    float wsum = 0.0;
