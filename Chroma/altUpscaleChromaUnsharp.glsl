...
vec4 hook()
{
	...
	for (float i = 1.0 - ceil(R); i <= ceil(R); ++i) {
		...
		color = PASS1_tex(vec2(base.x, base.y + i * PASS1_pt.y)).xy;
		csum += color * weight;
		wsum += weight;
...
vec4 hook()
{
	...
	for (float i = 1.0 - ceil(R); i <= ceil(R); ++i) {
		weight = get_weight(abs(i - f));
		color = PASS2_tex(vec2(base.x + i * PASS2_pt.x, base.y)).xy;
		csum += color * weight;
		wsum += weight;
		if (AR > 0.0 && i >= 0.0 && i <= 1.0) {
			lo = min(lo, color);
...
vec4 hook()
{
	...
	for (float i = 1.0; i <= R; ++i) {
		weight = get_weight(i);
		csum += (PASS3_tex(PASS3_pos + vec2(0.0, -i) * PASS3_pt).xy + PASS3_tex(PASS3_pos + vec2(0.0, i) * PASS3_pt).xy) * weight;
		wsum += 2.0 * weight;
	}
...
{
	float weight;
	vec2 csum = PASS4_tex(PASS4_pos).xy;
	float wsum = 1.0;
	for (float i = 1.0; i <= R; ++i) {
		weight = get_weight(i);
		csum += (PASS4_tex(PASS4_pos + vec2(-i, 0.0) * PASS4_pt).xy + PASS4_tex(PASS4_pos + vec2(i, 0.0) * PASS4_pt).xy) * weight;
		wsum += 2.0 * weight;
	}
	vec2 original = PASS3_tex(PASS3_pos).xy;
